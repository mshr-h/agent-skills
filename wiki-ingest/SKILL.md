---
description: Maintain the Obsidian Vault wiki by ingesting new source files from raw/ or tmp/ into wiki/, or by ingesting a Codex attachment only when the user explicitly names it as an attachment source. Use when the user says ingest, ingest-deep, インジェストして, 深く取り込んで, or asks Codex to add source material into the Vault wiki while preserving the rule that only wiki/ may be edited.
name: wiki-ingest
---
# Wiki Ingest
...
Use a Codex attachment path as a source only when the user explicitly says to treat that attachment as the source. Treat attachment paths as read-only: do not move, delete, normalize, copy into the Vault, or edit them. If the user does not explicitly authorize the attachment source, ask them to place the source under `raw/` or `tmp/`, or plan the ingest on that assumption.

Do not write full local attachment paths or old exception labels into wiki pages. When a wiki `## ソース` section needs to mention an attachment source, use a neutral display name such as `- 添付テキスト「<タイトル>」`.

Concept and entity pages must still be written from the original attachment text itself, not from the generated summary. In the final report, state that an attachment source was used and note that long-term source preservation is better handled by placing the source under `raw/` or `tmp/`.
...
1. Read the original source directly from `raw/` or `tmp/`, or from the explicitly authorized attachment source.
...
7. Report changed wiki files, any notable gaps, and whether the attachment source was used.
...
3. Write concept/entity content only from the original source, not from the generated summary. For attachment sources, the original source is the attachment text.
